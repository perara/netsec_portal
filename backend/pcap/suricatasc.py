...
class SuricataSocket:
    ...
    def __init__(self, socket_path, pcap_path, report_path, v_socket, v_pcap, v_report, verbose=False):
        self.FULL_SOCKET_PATH = socket_path
        self.FULL_PCAP_PATH = pcap_path
        self.FULL_REPORT_PATH = report_path

        self.VIRTUAL_PCAP_PATH = v_pcap
        self.VIRTUAL_REPORT_PATH = v_report

        self.sc = SuricataSC(self.FULL_SOCKET_PATH, verbose=verbose)
        ...
        try:
            while not self.is_connected:
                try:
                    ...
                    print("Unable to connect to socket %s: %s" % (self.FULL_SOCKET_PATH, err))
                    time.sleep(10)
        except SuricataReturnException as err:
            print("Unable to negotiate version with server: %s" % (err))
            exit(0)

    def queue_pcap(self, source_pcap):
        """
        :param id:
        :param pcap_path: Must be suricata path (the container path)
        :return:
        """
        virtual_source = os.path.join(self.VIRTUAL_PCAP_PATH, source_pcap)
...
if __name__ == "__main__":
    x = SuricataSocket("ntt_suricata", "socket", "suricata.socket", "pcaps", "reports", verbose=True)
